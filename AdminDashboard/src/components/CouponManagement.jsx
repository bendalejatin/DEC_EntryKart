...
const CouponManagement = () => {
  ...
  const visibleCoupons = coupons.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage);
  ...
  const handleFlatChange = (flatNo) => {
    ...
    const user = users.find(user => 
      ...
      user.society && user.society._id === selectedSociety
    );
  ...
  const handleEventChange = (e) => {
    ...
    if (eventObj) setExpiryDate(eventObj.date);
    else setExpiryDate("");
  };
  ...
  const handleCouponSubmit = async (e) => {
    ...
    try {
      let payload = { societyId: selectedSociety, code, expiryDate, eventId: selectedEvent, adminEmail };
      if (generateForAll) {
      ...
      console.error("❌ Error saving coupon:", error.response?.data || error.message);
  ...
  const handleEditCoupon = (coupon) => {
    ...
    const societyId = coupon.society && coupon.society._id ? coupon.society._id : coupon.society;
  ...
  return (
    <div className="coupon-management">
      ...
      <form className="coupon-form" onSubmit={handleCouponSubmit}>
        <label>Society:</label>
        <select value={selectedSociety} onChange={(e) => handleSocietyChange(e.target.value)}>
        ...
        <label>
          <input type="checkbox" checked={generateForAll} onChange={(e) => setGenerateForAll(e.target.checked)} />
        ...
        {!generateForAll && (
          <>
            <label>Flat:</label>
            <select value={selectedFlat} onChange={(e) => handleFlatChange(e.target.value)} disabled={!selectedSociety}>
        ...
        {editingCoupon && <button type="button" onClick={resetForm}>Cancel Edit</button>}
      </form>
                  ...
                  <tr key={coupon._id}>
                    ...
                    <td>
                      {coupon.society && coupon.society.name 
                        ? coupon.society.name 
                        : (societies.find(soc => soc._id === coupon.society)?.name || "N/A")}
                    </td>
                    <td>
                      {coupon.event && coupon.event.title 
                        ? coupon.event.title 
                        : (events.find(ev => ev._id === coupon.event)?.title || "N/A")}
                    </td>
                    <td>{coupon.expiryDate}</td>
                    <td>
                      {coupon.qrCode ? (
                        <img src={coupon.qrCode} alt="QR Code" style={{ width: "80px" }} />
                      ) : "N/A"}
                    </td>
                    <td>{coupon.status}</td>
                    <td>
                      <button className="edit-btn" onClick={() => handleEditCoupon(coupon)}>✏ Edit</button>
                      <button className="delete-btn" onClick={() => handleDeleteCoupon(coupon._id)}>🗑️ Delete</button>
                      <button className="scan-btn" onClick={() => handleScanCoupon(coupon)}>🔍 Manual Scan</button>
                    </td>
                  </tr>
                ))}
              </tbody>
