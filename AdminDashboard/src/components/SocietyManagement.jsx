...
const SocietyManagement = () => {
  ...
  const addSociety = async () => {
    ...
    try {
      ...
      alert(`❌ Failed to save society: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  const handleDeleteSociety = async (id) => {
    ...
    const confirmDelete = window.confirm("Are you sure you want to delete this society?");
    ...
    try {
      ...
      alert(`❌ Error deleting society: ${error.response?.data?.message || error.message}`);
    }
  };
        ...
        {societies.length > 0 ? (
          societies.map((society) => (
            <div key={society._id} className="society-card">
              ...
              <button className="edit-btn" onClick={() => handleEditSociety(society)}>
                ✏ Edit
              </button>
              {/* Set type="button" to prevent form submission */}
              <button type="button" className="delete-btn" onClick={() => handleDeleteSociety(society._id)}>
                🗑 Delete
              </button>
            </div>
          ))
